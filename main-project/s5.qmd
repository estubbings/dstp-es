...
## Plot map of UK wide collisions 2020 
```{r}
library(sf)
...
## Plot map of UK wide collisions 2021
...
## Plot map of UK wide collisions 2020 and 2021 side by side
...
## Filter to only those with >20 crashes within a 500m radius
...
# Consider differences between the two datasets
...
# Consider the places where collisions went down in 2021
...
pal <- colorNumeric(
  palette = "Reds",   # Can also try "RdYlGn", "OrRd", "PuBu"
  domain = hotspot_negative$diff,
  reverse = TRUE
)
...
leaflet(hotspot_negative) |>
  ...
  addLegend(
    ...
    title = "Decrease in Collisions",
    labFormat = labelFormat(transform = abs),  # makes labels positive
    opacity = 1
  )
```

# Consider places where collisions increased the most 2020-2021

```{r}

hotspot_positive <- hotspot_diff_sf |>
  filter(diff > 0)

# Create colour palette
pal2 <- colorNumeric(
  palette = "Blues",   # Can also try "RdYlGn", "OrRd", "PuBu"
  domain = hotspot_positive$diff
)

# Plot on an interactive leaflet map
library(leaflet)

leaflet(hotspot_positive) |>
  addProviderTiles("CartoDB.Positron") |>
  addCircleMarkers(
    radius = ~abs(diff) * 0.5,
    color = ~pal2(diff),
