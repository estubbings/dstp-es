...
collisions = get_stats19(year = 2022, type = "collisions")
```

```{r}
#| label: download-stats191
#| eval: false 
...
# Impute order(..., na.last = TRUE, decreasing = FALSE) remove missing values
...
collisions19 = get_stats19(year = 2019, type = "collisions"
)
...
3. Create a simple plot showing number of collisions by severity
...
collisions19 |>
  ...
  summarise(count = n()) |>
  ggplot(aes(x = accident_severity, y = count, fill = accident_severity)) +
  geom_col() +
  labs(
    title = "Number of Collisions by Severity (2019)",
    x = "Accident Severity",
